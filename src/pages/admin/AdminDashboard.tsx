...
export function AdminDashboard() {
  ...
  return (
    <div className="px-10 py-10">
      <h1 className="font-display text-[32px] text-espresso font-normal mb-8">Dashboard</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
            {stats.slice(0, 4).map((s) => (
              <div key={s.label} className="bg-white p-5">
                <div className="font-body text-[11px] tracking-wide uppercase text-warmgray mb-2">{s.label}</div>
                <div className="font-display text-2xl text-espresso">{s.value}</div>
              </div>
            ))}
          </div>

          <div className="flex items-center justify-between mb-4">
            <div className="font-body text-xs tracking-widest uppercase text-espresso">Recent Orders</div>
            <Link to="/admin/orders" className="font-body text-xs text-sage underline">
              View All
            </Link>
          </div>
          <div className="bg-white">
            {orders.length === 0 ? (
              ...
              orders.slice(0, 8).map((o) => (
                <div key={o.id} className="flex items-center justify-between px-6 py-4 border-b border-espresso/10 last:border-0">
                  <div>
                  ...
                  <div className={`font-body text-xs uppercase tracking-wide ${statusColor[o.status] ?? 'text-espresso'}`}>
                    {o.status}
                  </div>
